...
/// <summary>
/// Test request sender implementation, implemented with async in mind.
/// </summary>
internal class AsyncTestRequestSender
{
    ...
    private readonly IAsyncCommunicationEndPoint _communicationEndpoint;
    ...
    public AsyncTestRequestSender(
        IAsyncCommunicationEndpointFactory communicationEndpointFactory,
        ...
        int clientExitedWaitTime)
    {
        ...
        _communicationEndpoint = CreateCommunicationEndPoint(communicationEndpointFactory, testhostConnectionInfo);
    }

    public bool CloseConnectionOnOperationComplete { get; set; } = true;

    /// <inheritdoc />
    public async Task<ConnectionInfo> InitializeCommunicationAsync(CancellationToken cancellationToken)
    {
        EqtTrace.Verbose("AsyncTestRequestSender.InitializeCommunication: initialize communication. ");

        var realAddress = await _communicationEndpoint.ListenAsync(cancellationToken);

        // TOOD: Maybe move all this info to the communication endpoint, and make ListenAsync return just task?
        return new ConnectionInfo(realAddress, _connectionInfo.Role, _connectionInfo.Transport);
    }

    /// <inheritdoc />
    public async Task<bool> WaitForRequestHandlerConnectionAsync(int connectionTimeout, CancellationToken cancellationToken)
    {
        var sw = Stopwatch.StartNew();
        EqtTrace.Verbose("AsyncTestRequestSender.WaitForRequestHandlerConnection: waiting for connection with timeout: {0}.", connectionTimeout);

        _communicationEndpoint.SetHandler(HandleRequestAsync, context: this);

        try
        {
            //ERRR: this will wait forever, we need to return the address first so we can start the other side with the correct address.
            await _communicationEndpoint.ConnectAsync(cancellationToken);
            _connected = true;
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.WaitForRequestHandlerConnection: waiting took {0} ms, with timeout {1} ms, and result {2}, which is {3}.", sw.ElapsedMilliseconds, connectionTimeout, waitIndex, waitIndex == 0 ? "success" : "failure");
    ...
    public void CheckVersionWithTestHost()
    {
        ...
        _onMessageReceived = (sender, args) =>
        {
            ...
            EqtTrace.Verbose("AsyncTestRequestSender.CheckVersionWithTestHost: onMessageReceived received message: {0}", message);
        ...
        try
        {
            ...
            EqtTrace.Verbose("AsyncTestRequestSender.CheckVersionWithTestHost: Sending check version message: {0}", data);
    ...
    public void InitializeDiscovery(IEnumerable<string> pathToAdditionalExtensions)
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.InitializeDiscovery: Sending initialize discovery with message: {0}", message);
    ...
    public void DiscoverTests(DiscoveryCriteria discoveryCriteria, ITestDiscoveryEventsHandler2 discoveryEventsHandler)
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.DiscoverTests: Sending discover tests with message: {0}", message);
    ...
    public void SendDiscoveryAbort()
    {
        ...
        {
            EqtTrace.Verbose("AsyncTestRequestSender.SendDiscoveryAbort: Operation is already complete. Skip error message.");
            return;
        }

        _isDiscoveryAborted = true;
        EqtTrace.Verbose("AsyncTestRequestSender.SendDiscoveryAbort: Sending discovery abort.");
    ...
    public void InitializeExecution(IEnumerable<string> pathToAdditionalExtensions)
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.InitializeExecution: Sending initialize execution with message: {0}", message);
    ...
    public void StartTestRun(TestRunCriteriaWithSources runCriteria, IInternalTestRunEventsHandler eventHandler)
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.StartTestRun: Sending test run with message: {0}", message);
    ...
    public void SendTestRunCancel()
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.SendTestRunCancel: Sending test run cancel.");
    ...
    public void SendTestRunAbort()
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.SendTestRunAbort: Sending test run abort.");
    ...
    public void EndSession()
    {
        if (!IsOperationComplete())
        {
            EqtTrace.Verbose("AsyncTestRequestSender.EndSession: Sending end session.");
    ...
    public void OnClientProcessExit(string stdError)
    {
        ...
        EqtTrace.Info($"AsyncTestRequestSender.OnClientProcessExit: Test host process exited. Standard error: {stdError}");
    ...
    private void OnExecutionMessageReceived(MessageReceivedEventArgs messageReceived, IInternalTestRunEventsHandler testRunEventsHandler)
    {
        try
        {
            ...
            EqtTrace.Verbose("AsyncTestRequestSender.OnExecutionMessageReceived: Received message: {0}", rawMessage);
            ...
            switch (message.MessageType)
            {
                ...
                case MessageType.LaunchAdapterProcessWithDebuggerAttached:
                    ...
                    var data =
                        _dataSerializer.SerializePayload(
                            ...
                            processId,
                            _protocolVersion);
                    EqtTrace.Verbose("AsyncTestRequestSender.OnExecutionMessageReceived: Sending LaunchAdapterProcessWithDebuggerAttachedCallback message: {0}", data);
                ...
                case MessageType.AttachDebugger:
                    ...
                    EqtTrace.Verbose("AsyncTestRequestSender.OnExecutionMessageReceived: Sending AttachDebugger with message: {0}", message);
    ...
    private void OnDiscoveryMessageReceived(ITestDiscoveryEventsHandler2 discoveryEventsHandler, MessageReceivedEventArgs args)
    {
        try
        {
            ...
            EqtTrace.Verbose("AsyncTestRequestSender.OnDiscoveryMessageReceived: Received message: {0}", rawMessage);
            ...
            if (data is null)
            {
                EqtTrace.Error("AsyncTestRequestSender.OnDiscoveryMessageReceived: Deserialized message is null: {0}", rawMessage);
    ...
    private void OnDiscoveryAbort(ITestDiscoveryEventsHandler2 eventHandler, Exception? exception, bool getClientError)
    {
        if (IsOperationComplete())
        {
            EqtTrace.Verbose("AsyncTestRequestSender.OnDiscoveryAbort: Operation is already complete. Skip error message.");
            return;
        }

        EqtTrace.Verbose("AsyncTestRequestSender.OnDiscoveryAbort: Set operation complete.");
        SetOperationComplete();
        ...
        {
            EqtTrace.Error("AsyncTestRequestSender.OnDiscoveryAbort: Aborting test discovery because {0}.", reason);
            LogErrorMessage(string.Format(CultureInfo.CurrentCulture, CommonResources.AbortedTestDiscoveryWithReason, reason));
        }
        else
        {
            EqtTrace.Error("AsyncTestRequestSender.OnDiscoveryAbort: Aborting test discovery.");
    ...
    {
        EqtTrace.Verbose("AsyncTestRequestSender.GetAbortErrorMessage: Exception: " + exception);
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.GetAbortErrorMessage: Client has disconnected. Wait for standard error.");
        ...
        if (_clientExited.Wait(_clientExitedWaitTime))
        {
            ...
            EqtTrace.Info("AsyncTestRequestSender.GetAbortErrorMessage: Received test host error message.");
        ...
        {
            EqtTrace.Info("AsyncTestRequestSender.GetAbortErrorMessage: Timed out waiting for test host error message.");
    ...
    private void LogErrorMessage(string message)
    {
        ...
        {
            EqtTrace.Error("AsyncTestRequestSender.LogErrorMessage: Message event handler not set. Error: " + message);
            return;
        }
    ...
    private void SetOperationComplete()
    {
        ...
        EqtTrace.Verbose("AsyncTestRequestSender.SetOperationComplete: Setting operation complete.");

        _communicationEndpoint.Stop();
        Interlocked.CompareExchange(ref _operationCompleted, 1, 0);
    }

    private static IAsyncCommunicationEndPoint CreateCommunicationEndPoint(IAsyncCommunicationEndpointFactory communicationEndpointFactory, TestHostConnectionInfo testhostConnectionInfo)
    {
        ...
        var endpoint = testhostConnectionInfo.Endpoint;
        var role = testhostConnectionInfo.Role == ConnectionRole.Host
            ? ConnectionRole.Client
            : ConnectionRole.Host;
        var transport = testhostConnectionInfo.Transport;

        // TODO: move this to the factory / server ctor.
        ...
        return communicationEndpointFactory.Create("TestRequestSender", endpoint, role, transport);
    }
}

internal class ConnectionInfo
{
    public ConnectionInfo(string address, ConnectionRole role, Transport transport)
    {
        Address = address;
        Role = role;
        Transport = transport;
