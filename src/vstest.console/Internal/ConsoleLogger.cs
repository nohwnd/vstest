...
namespace Microsoft.VisualStudio.TestPlatform.CommandLine.Internal
{
    ...
    internal class ConsoleLogger : ITestLoggerWithParameters
    {
        ...
        public void Initialize(TestLoggerEvents events, string testRunDirectory)
        {
            ...
            events.SourceRunComplete += this.SourceRunCompleteHandler;
        ...
        private void SourceRunCompleteHandler(object sender, TestResultEventArgs e)
        {
            ...
            var resultsFromSource = this.leafTestResults.Select(p=>p.Value).Where(p => p.TestCase.Source == e.Result.TestCase.Source).ToList();
            var source = e.Result.TestCase.Source;
            var sourceSummary = new SourceSummary();

            foreach (var result in resultsFromSource)
            {
            ...
            var sourceName = source.Split('\\').Last();
            ...
            Output.Information(false, CommandLineResources.TestRunSummaryAssemblyAndFramework, sourceName, frameworkString);
        }
    ...
    private void TestRunCompleteHandler(object sender, TestRunCompleteEventArgs e)
    {
        ...
        foreach (var r in leafTestResults.Select(p => p.Value))
        {
            switch (r.Outcome)
            {
                case TestOutcome.Passed:
